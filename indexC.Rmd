...
```{r, results='asis'}
intro_text <- glue("<font size=2.5>基因的表达是如何受到调控，同时又是如何影响植物表型的？我希望结合传统遗传学、比较基因组学和多组学数据挖掘等手段，以玉米和小麦为模型研究作物在生长发育过程和胁迫条件下的基因表达调控机制，鉴定关键的顺式调控元件和反式作用因子用于作物改良。此外，我将借助大数据分析和多组学数据整合构建植物基因调控网络，训练机器学习模型用于预测基因的时空特异性表达模式，对作物进行精准育种和设计。欢迎对生信感兴趣的同学加入我们团队！</font>")
